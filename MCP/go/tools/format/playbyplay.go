...
func PlaybyplayHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		hometeamVal, ok := args["hometeam"]
		if !ok {
			return mcp.NewToolResultError("Missing required path parameter: hometeam"), nil
		}
		hometeam, ok := hometeamVal.(string)
		if !ok {
			return mcp.NewToolResultError("Invalid path parameter: hometeam"), nil
		}
		url := fmt.Sprintf("%s/%s/PlayByPlay/%s/%s/%s", cfg.BaseURL, format, season, week, hometeam)
...
func CreatePlaybyplayTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_format_PlayByPlay_season_week_hometeam",
		mcp.WithDescription("Play By Play"),
		...
		mcp.WithString("hometeam", mcp.Required(), mcp.Description("Abbreviation of the home team. Example: <code>WAS</code>.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    PlaybyplayHandler(cfg),
	}
}
